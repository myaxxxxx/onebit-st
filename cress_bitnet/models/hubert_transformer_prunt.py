...
from .quantizelinear import QuantizeLinear, replace_linear_layers_with_quantize_recursive

from .bit_linear import replace_linears_in_hf, BitLinear

logger = logging.getLogger(__name__)

@register_model("quant_transformer")
class HubertTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        # replace_linears_in_hf(model.encoder.transformer_layers)
        if mt_pretrained_path is not None and Path(mt_pretrained_path).exists():
            
            for submodel in model.decoder.layers:
                replace_linears_in_hf(submodel.encoder_attn)  
            print(model)
    
            # replace_linears_in_hf(model.encoder.hubert_model)
            # exit(
...
# add hubert
        ...
        hubert_args.model["_name"] = "quant_hubert"
        print(hubert_args)
...
@register_model_architecture(model_name="quant_transformer", arch_name="quant_transformer")
...
@register_model_architecture(model_name="quant_transformer", arch_name="quant_transformer_postln")
