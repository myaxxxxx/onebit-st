...
# add
        ...
        if head_layer_z is not None:
            attn = attn.mul(head_layer_z)        
        if not inference and attn.sum().eq(0).item():
            # attn = attn + input_tensor
            id = 1  # need update  hidden_states = hidden_states + input_tensor
        else:
            if hidden_z is not None:
                attn = attn.mul(hidden_z)
